...
from amherst_core.commence_types import CursorName
from amherst_core.models import AmherstShipableBase, get_table_model
from amherst_core.models._base import AmherstContainer
...
async def pycommence_fetch(
    ...
    if row_id:
        ...
        model_type = get_table_model(q.csrname)
        record = model_type(**row.data)
        container = AmherstContainer(row_info=row.row_info, data=record)
        return container
    return None
...
async def pycommence_fetch_f_info(
    ...
    return AmherstContainer(row_info=row_info, data=model_type.model_validate(row))
...
async def pycommence_search(
    ...
    if container := await pycommence_fetch(q, pycmc):
        records, more = [container.data], None
    else:
