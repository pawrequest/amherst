...
from amherst.back.backend_search_paginate import Pagination, log_action, log_action_sync
from pycommence.filters import FieldFilter, Logic, SortOrder
from pycommence.pycmc_types import MoreAvailable
from amherst.models.amherst_models import AMHERST_TABLE_MODELS, AmherstHire, AmherstSale
...
class SearchRequest2(BaseModel):
    ...
    def from_body(
        ...
        logger.info(f'{cls.__name__}.from_body: {res}')
...
class SearchDict(BaseModel):
    ...
    def val_(self):
        # if not self.customer_name and not self.filters:
        #     raise ValueError('Must have either customer_name or filters')
