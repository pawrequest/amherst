...
# from amherst.models.maps import AmherstTableName, record_model
#
#
...
#     logger.debug(f'pycmc_f_path({csrname=})')
...
# ) -> SearchResponse:
#     logger.debug(f'pycommence_response({search_request=})')
#     with pycommence_context(csrname=search_request.csrname) as pycmc:
...
# ) -> SearchResponse:
#     logger.debug(f'search_f_path({resp=})')
#     return resp
#
#
...
#         row_id: str = Path(...),
#         pycmc: PyCommence = Depends(pycmc_f_path),
#         record_type=Depends(record_model),
#
# ) -> AMHERST_TABLE_MODELS:
#     logger.debug(f'row_from_path_id({csrname=}, {row_id=})')
