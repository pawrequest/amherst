...
class CustomerFields2(FieldsABC):
    NAME = 'Name'
    ...
    match tablename:
        case 'Hire':
            return FilterArray.from_filters(
                *hires_in_range_fils(
                    date(2023, 5, 1), date(2024, 12, 31)
                )
            )
        case 'Sale':
            return FilterArray(
                filters={
                    1: CmcFilter(cmc_col=SaleFields.DATE_ORDERED, condition=ConditionType.AFTER, value='6 months ago'),
                }
            )
        case _:
...
def hires_in_range_fils(start_date: date, end_date: date):
    return (
        ...
        CmcFilter(cmc_col=HireFields.SEND_OUT_DATE, condition=ConditionType.AFTER, value=to_cmc_date(start_date)),
    )
