...
import amherst.front.am_styles
...
from shipr.ship_ui.forms import big_form_fields
...
async def right_col3(manager):
    return c.Div(
        class_name='col',
        components=[
            c.Form(
                form_fields=await big_form_fields(manager.state),
                submit_url=f'/api/forms/big/{manager.id}',

            ),
            await manual_entry_modal(manager)
        ]
    )


async def manual_entry_modal(manager: managers.BookedManager) -> c.Div:
    return c.Div(
        components=[
            c.Button(
                text='Manual Entry',
