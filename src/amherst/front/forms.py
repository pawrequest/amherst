...
#         c.FireEvent(
#             event=e.GoToEvent(
#                 url=f'/ship/update/{shiprec_id}/{partial.model_dump_64()}'
#             )
#         )
#     ]
...
#     c.FireEvent(
#         event=e.GoToEvent(
#             url=f'/sl/pcneighbours/{shiprec_id}/{form.fetch_address_from_postcode}'
#         )
#     )
# ]
...
# ) -> list[c.AnyComponent]:
#     man_in = await support.get_manager(shiprec_id, session)
#     man_in.shipment.candidates = el_client.get_candidates(form.fetch_address_from_postcode)
#     session.add(man_in)
#     session.commit()
#
...
#         )
#
#         # return await shipping_page.ship_page(shiprec=man_in, alert_dict=alertdict)
#
#         # return [c.FireEvent(event=e.GoToEvent(url=f'/ship/view/{shiprec_id}'))]
#
#     # return await ship_page_2.shipping_page(shiprec_id=shiprec_id, session=session)
#     return [c.FireEvent(event=events.GoToEvent(url=f'/ship/view/{shiprec_id}'))]
#
#     # return [
#     #     c.FireEvent(
#     #         event=e.GoToEvent(
...
# @router.post('/boxes/{shiprec_id}', response_model=FastUI, response_model_exclude_none=True)
...
# @router.post('/contact/{shiprec_id}', response_model=FastUI, response_model_exclude_none=True)
# async def contact_post(
#         shiprec_id: int,
#         form: Annotated[ContactForm, fastui_form(ContactForm)],
#         session=fastapi.Depends(am_db.get_session),
# ) -> list[c.AnyComponent]:
#     man_in = await get_manager(shiprec_id, session)
...
#         c.FireEvent(
#             event=e.GoToEvent(
#                 url=f'/ship/update/{shiprec_id}/{man_in.shipment.update_dump_64(contact=contact)}'
#             ),
#         )
#     ]
...
#         c.FireEvent(
#             event=e.GoToEvent(
#                 url=f'/ship/update/{shiprec_id}/{states.ShipmentPartial(boxes=boxes).model_dump_64()}'
#             ),
#         )
#     ]
...
#         c.FireEvent(
#             event=e.GoToEvent(
#                 url=f'/ship/update/{shiprec_id}/{man_in.shipment.update_dump_64_dict(address=address, contact=contact)}'
#             ),
#         )
#     ]
...
#         c.FireEvent(
#             event=e.GoToEvent(
#                 url=f'/ship/update/{shiprec_id}/{man_in.shipment.update_dump_64_dict(update={**form_data})}'
#             ),
#         )
#     ]
...
# @router.post('/address/{shiprec_id}', response_model=FastUI, response_model_exclude_none=True)
...
#         c.FireEvent(
#             event=e.GoToEvent(
#                 url=f'/ship/update/{shiprec_id}/{man_in.shipment.update_dump_64(address=address)}'
#             ),
#         )
#     ]


# @router.post('/full/{shiprec_id}', response_model=FastUI, response_model_exclude_none=True)
# async def full_post(
#         form: Annotated[ship_forms.FullForm, fastui_form(ship_forms.FullForm)],
#         shiprec_id: int,
#         el_client: ELClient = fastapi.Depends(am_db.get_el_client),
...
async def email_post(
    ...
    #     shiprec=managers.MANAGER_IN_DB.get(shiprec_id, session),
    #     invoice=invoice,
    #     label=label,
    #     missing=missing_kit,
    # )
