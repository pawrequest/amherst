...
#     record: AMHERST_TABLE_MODELS, shipment: SHIPMENT_TYPES, shipment_response: ShipmentResponsePF
# ) -> dict[str, Any]:
#     """Adds tracking numbers and link."""
...
#
#
# async def tracking_link_update(record: AmherstHire, shipment: Shipment, shipment_response: ShipmentBookingResponseAgnost):
#     aliases = await get_alias(record)
...
#
#
# async def make_hire_update_extra2(record: AmherstHire, shipment: ShipmentPF):
#     shipdir = shipment.direction
#     if shipdir in [ShipDirection.INBOUND, ShipDirection.DROPOFF]:
...
#
#
# async def make_hire_update_in(record: AmherstHire, shipment: ShipmentPF):
#     aliases = HireAliases
#     ret_notes = f'{date.today().strftime('%d/%m')}: pickup arranged for {shipment.shipping_date.strftime('%d/%m')}\r\n{record.return_notes}'
#     return {
#         aliases.ARRANGED_IN: 'True',
